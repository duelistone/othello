...
int alphabeta(Board b, int depth, Side s, int alpha = INT_MIN, int beta = INT_MAX, bool prevPass = false, int noPC = 0) {
	//~ abCalls++;
	...
	if (depth <= 0) {
		...
		if (s == BLACK) {
			...
			if (eval == INT_MIN) return -15 * mobilityWeight - mobilityBoost;
			return eval;
		}
		else {
			...
			if (eval == INT_MAX) return 15 * mobilityWeight + mobilityBoost;
			return eval;
		}
	}
	if (alpha == INT_MIN || beta == INT_MAX || depth <= 4 || noPC > 0) goto skip_pc;
	{
		int sigma = SIGMA + __builtin_popcountll(b.taken) / 10;
		int eval = alphabeta(b, 0, s, alpha - sigma - 2, beta + sigma + 2);
		if (eval >= beta + sigma  + 2 && alphabeta(b, depth - 6, s, beta + sigma + 1, beta + sigma + 2, false, 2) >= beta + sigma + 2) return beta;// {if (depth >= 10) cerr << depth << endl; return beta;}
		if (eval <= alpha - sigma - 2 && alphabeta(b, depth - 6, s, alpha - sigma - 2, alpha - sigma - 1, false, 2) <= alpha - sigma - 2) return alpha;//{if (depth >= 10) cerr << depth << endl; return alpha;}
		if (eval >= beta + sigma  + 1 && alphabeta(b, depth - 4, s, beta + sigma, beta + sigma + 1, false, 2) >= beta + sigma + 1) return beta;// {if (depth >= 10) cerr << depth << endl; return beta;}
		if (eval <= alpha - sigma - 1 && alphabeta(b, depth - 4, s, alpha - sigma - 1, alpha - sigma, false, 2) <= alpha - sigma - 1) return alpha;//{if (depth >= 10) cerr << depth << endl; return alpha;}
		if (eval >= beta + sigma && alphabeta(b, depth - 2, s, beta + sigma - 1, beta + sigma, false, 1) >= beta + sigma) return beta;// {if (depth >= 10) cerr << depth << endl; return beta;}
		if (eval <= alpha - sigma && alphabeta(b, depth - 2, s, alpha - sigma, alpha - sigma + 1, false, 1) <= alpha - sigma) return alpha;//{if (depth >= 10) cerr << depth << endl; return alpha;}
	}
	skip_pc:
	...
	if (!legalMoves) {
		...
		return alphabeta(b, depth - 1, other_side(s), alpha, beta, true, noPC - 1);
	}
	int besti = -1;
	if (depth > 3 && s && um4->count(bws)) {
			...
			v = alphabeta(b.doMoveOnNewBoard(guess, s), depth - 1, other_side(s), alpha, beta, false, noPC - 1);
			besti = guess;
			alpha = (v > alpha) ? v : alpha;
			while (legalMoves && alpha < beta) {	
				int index = __builtin_clzl(legalMoves);
				legalMoves ^= SINGLE_BIT[index];
				int val = alphabeta(b.doMoveOnNewBoard(index, s), depth - 1, other_side(s), alpha, beta, false, noPC - 1);
			...
			int index = __builtin_clzl(legalMoves);
			legalMoves ^= SINGLE_BIT[index];
			v = alphabeta(b.doMoveOnNewBoard(index, s), depth - 1, other_side(s), alpha, beta, false, noPC - 1);
			besti = index;
			alpha = (v > alpha) ? v : alpha;
			while (legalMoves && alpha < beta) {	
				int index = __builtin_clzl(legalMoves);
...
int alphabeta_find_move(Board b, int depth, Side s, int alpha = INT_MIN, int beta = INT_MAX, bool prevPass = false) {
	//~ abCalls++;
...
int pvsCalls = 0;
int pvs(Board b, int depth, Side s, int alpha = INT_MIN, int beta = INT_MAX, bool prevPass = false, int noPC = 0) {
	pvsCalls++;
	...
	// 5: 8.6 sec
	if (depth < 6) {
		return alphabeta(b, depth, s, alpha, beta, prevPass, noPC);
	}
	...
	if (alpha == INT_MIN || beta == INT_MAX || depth <= 4 || noPC > 0) goto skip_pc;
	{
		int sigma = SIGMA + __builtin_popcountll(b.taken) / 10;
		int eval = alphabeta(b, 0, s, alpha - SIGMA - 2, beta + SIGMA + 2);
		if (eval >= beta + sigma  + 2 && pvs(b, depth - 6, s, beta + sigma + 1, beta + sigma + 2, false, 2) >= beta + sigma + 2) return beta;// {if (depth >= 10) cerr << depth << endl; return beta;}
		if (eval <= alpha - sigma - 2 && pvs(b, depth - 6, s, alpha - sigma - 2, alpha - sigma - 1, false, 2) <= alpha - sigma - 2) return alpha;//{if (depth >= 10) cerr << depth << endl; return alpha;}
		if (eval >= beta + sigma  + 1 && pvs(b, depth - 4, s, beta + sigma, beta + sigma + 1, false, 2) >= beta + sigma + 1) return beta;// {if (depth >= 10) cerr << depth << endl; return beta;}
		if (eval <= alpha - sigma - 1 && pvs(b, depth - 4, s, alpha - sigma - 1, alpha - sigma, false, 2) <= alpha - sigma - 1) return alpha;//{if (depth >= 10) cerr << depth << endl; return alpha;}
		if (eval >= beta + sigma && pvs(b, depth - 2, s, beta + sigma - 1, beta + sigma, false, 1) >= beta + sigma) return beta;// {if (depth >= 10) cerr << depth << endl; return beta;}
		if (eval <= alpha - sigma && pvs(b, depth - 2, s, alpha - sigma, alpha - sigma + 1, false, 1) <= alpha - sigma) return alpha;//{if (depth >= 10) cerr << depth << endl; return alpha;}
	}
	//~ Timer tim;
	/*
	double sigma, static_eval;
	if (alpha == INT_MIN || beta == INT_MAX || depth < 8) goto skip_pc;
	{
		int bound, secondDepth;
		sigma = SIGMA;// + totalDiscs / 15;
		static_eval = alphabeta(b, 0, s, -PERCENTILE * (sigma + 3) + alpha, PERCENTILE * (sigma + 3) + beta);
		//~ Timer tim1;
		secondDepth = depth - 8;
		if (secondDepth >= 4) {
				bound = round(PERCENTILE * (sigma + 3) + beta);
				if (pvs(b, secondDepth - 4, s, bound - 1, bound) >= bound && pvs(b, secondDepth, s, bound - 1, bound) >= bound) return beta; // {cerr << "pruning " << depth - secondDepth << endl; tim.end(); return beta;}
				bound = round(-PERCENTILE * (sigma + 3) + alpha);
				if (pvs(b, secondDepth - 4, s, bound, bound + 1) <= bound && pvs(b, secondDepth, s, bound, bound + 1) <= bound) return alpha;
		}
		//~ tim1.end("block 1 ");
		Timer tim2;
		secondDepth = depth - 6;
		...
		if (secondDepth >= 4) {
				...
				if (pvs(b, secondDepth - 4, s, bound - 1, bound) && pvs(b, secondDepth, s, bound - 1, bound) >= bound) return beta; // {cerr << "pruning " << depth - secondDepth << endl; tim.end();return beta;}
				bound = round(-PERCENTILE * (sigma + 1) + alpha);
				if (pvs(b, secondDepth - 4, s, bound, bound + 1) <= bound && pvs(b, secondDepth, s, bound, bound + 1) <= bound) return alpha;
		}
		//~ tim3.end("block 3 ");
		//~ cerr << "Nothing" << endl; tim.end();
	}*/
	skip_pc:
	...
	if (!legalMoves) {
		...
		return pvs(b, depth - 1, other_side(s), alpha, beta, true, noPC - 1);
	}
	...
	if (s && um4->count(bws)) {
		...
		lm2 = legalMoves & X_SQUARES;
		Board b2 = b.doMoveOnNewBoard(index, s);
		v = pvs(b2, depth - 1, other_side(s), alpha, beta, false, noPC - 1);
		alpha = (v > alpha) ? v : alpha;
		if (alpha >= beta) {
			return alpha;
		}
		...
		while (lm1 && alpha < beta) {	
			...
			lm1 ^= SINGLE_BIT[index];
			Board b2 = b.doMoveOnNewBoard(index, s);
			v = pvs(b2, depth - 1, other_side(s), alpha, alpha + 1, false, noPC - 1);
			if (v >= alpha + 1) {
				v = pvs(b2, depth - 1, other_side(s), v, beta, false, noPC - 1);
				besti = index;
				alpha = v;
			}
		}
		...
		while (lm1 && alpha < beta) {	
			...
			if (v <= beta - 1) {
				v = pvs(b2, depth - 1, other_side(s), alpha, v, false, noPC - 1);
				besti = index;
				beta = v;
			}
		}
...
pair<int, int> main_minimax_aw(Board b, Side s, int depth, int guess = -1) {
	...
	//~ e = alphabeta(b, depth - 8, s);
	
	for (int i = 18; i > 8; i -= 2) {
		e = alphabeta(b, depth - i, s, INT_MIN, INT_MAX);
		cerr << "Finished depth " << depth - i << " search: " << e << ' ' << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count() << endl;
	}
	e = pvs(b, depth - 8, s, INT_MIN, INT_MAX, false, 4);
	cerr << "Finished depth " << depth - 8 << " search: " << e << ' ' << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count() << endl;
	...
	// Denominator 20: 2:21
	// Denominator 25: 2:28
	// Denominator 11:       21:10
	// Denominator 10: 2:14! 20:84
	// Denominator 9:        31:29??
	// Denominator 8: 2:13!  26:47
	// Denominator 7: 2:13 
	// Denominator 6: 2:13
	// Denominator 5: 2:14
	d = abs(e) / 10 + 3;
	...
	try_again5:
	um5->clear();
	e = pvs(b, depth - 6, s, lower, upper, false, 4);
	...
	try_again4:
	um5->clear();
	e = alphabeta(b, depth - 4, s, lower, upper, false, 4);
	if (e <= lower && lower != INT_MIN) {
		cerr << "Recalculating (failed low)" << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count() << endl;
		lower -= (d - 1) * counter;
		counter++;
	...
	try_again3:
	um5->clear();
	e = pvs(b, depth - 2, s, lower, upper, false, 4);
	if (e <= lower && lower != INT_MIN) {
		cerr << "Recalculating (failed low)" << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count() << endl;
		lower -= (d - 1) * counter;
		counter++;
	...
	um5->clear();
	pair<int, int> result;
	e = pvs(b, depth, s, lower, upper, false, 4);
	if (e <= lower && lower != INT_MIN) {
		cerr << "Recalculating (failed low)" << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count() << endl;
		lower -= (d - 1) * counter;
		counter++;
	...
	BoardWithSide bws(b.taken, b.black, s);
	if (um4->count(bws)) result = make_pair((*um4)[bws], e);
	else result = make_pair(alphabeta_find_move(b, depth, s, e - 1, e + 1), e);
	cerr << "Finished depth " << depth << " search: " << e << ' ' << chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count() << endl;
	return result;
}
