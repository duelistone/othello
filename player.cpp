...
Move *Player::doMove(Move *opponentsMove, int msLeft) {
	...
	function<pair<int, int>(Board, int, Side, bool)> minimax = [&] (Board b, int depth, Side s, bool prevPass) -> pair<int, int> {
		// Base case
		if (depth == 0) {
			b.evaluate();
    ...
    pair<int, int> p = minimax(currBoard, depth, side, false);
    int besti = p.first;
    /*
